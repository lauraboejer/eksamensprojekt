...
//opretter funktion for komponenten, som håndterer listevisning af begivenheder som den aktuelle bruger har tilmeldt sig
export default function RegisteredHobbies({ navigation }) {
    ...
    if (!hobbies) {
        ...
        //opretter en funktion der itererer gennemm hobbies-array'et og finder de begivenheder, hvor registered er lig den aktuelle bruger
        function GetHobbies() {
            return (
                    ...
                    data.forEach(function(hobby){
                        hobby.forEach(function(reg){
                            reg.forEach(function(user){
                                if(user.val().toString() === email) { //sætter user.val() til en streng, da den ellers er et objekt
        ...
        function handleSelectHobby(id, item) {
            const hobby = [id, item];
            return (
                navigation.navigate('Registered Details', { hobby }) //navigerer brugeren til detaljevisning for den valgte begivenhed
            );
        }
        //returnerer view'et for RegisteredHobbies-komponenten
        return (
